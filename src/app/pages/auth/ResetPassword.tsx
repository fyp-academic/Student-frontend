...
export default function ResetPassword() {
  ...
  const fieldCls = (hasErr: boolean) =>
    `w-full px-4 py-3 rounded-md border text-step-2 text-ink outline-none transition-all placeholder:text-ink-2/60 pl-10 pr-10 ${
      hasErr
  ...
  if (!token || !email) {
    return (
      <div className="text-center">
        ...
        <h1 className="font-display ed-display text-step-5 text-ink mb-2">Invalid Reset Link</h1>
        <p className="text-step-2 text-ink-2 mb-6">The password reset link is invalid or has expired.</p>
        <Link
          to="/forgot-password"
          className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-ink hover:bg-clay-deep text-paper font-semibold text-step-2 transition-colors duration-300"
        >
  ...
  return (
    <>
      ...
      <h1 className="font-display ed-display text-step-6 text-ink mb-2">Reset password</h1>
      <p className="text-step-2 text-ink-2 mb-7">Create a new password for <strong>{email}</strong></p>
      ...
      {success ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <div>
            <label className="block text-step-1 font-medium text-ink-2 mb-1.5">New password</label>
